...
export default {
  ...
  methods: {
    ...
    normalizeHorizontal(next) {
      ...
      this.allowBodyScroll = !shouldScroll;
      if (shouldScroll) {
        (this.left = next), (this.hMovement = (next / size.viewerWidth) * 100);
    ...
    onChangePosition(movement, orientation) {
      this.calculateSize(() => {
        ...
        if (orientation === "vertical")
          this.normalizeVertical(next * this.viewerHeight);
        if (orientation === "horizontal")
          this.normalizeHorizontal(next * this.viewerWidth);
      });
    },
    // 滚动条正在拖拽
    onScrollbarDragging() {
    ...
    calculateSize(cb) {
      ...
      if (
        size.wrapperHeight !== this.wrapperHeight ||
